...
const UserCard: FC<IProps> = ({
  ...
  const ratingText =
    userDetails && userDetails?.rating && userDetails.rating > 0
      ? userDetails.rating
      : "No ratings yet";
  const usernameText =
    userDetails && userDetails.username ? userDetails?.username : "Error";
  ...
  return (
    <Card className="w-full p-8" isBlurred={true}>
      <CardHeader className="flex justify-between">
        <span className="sm:text-3xl text-xl text-muted-foreground">
          Profile
        </span>
      ...
      <CardBody className="flex sm:align-start align-center">
        <div className="sm:grid sm:grid-rows-1 sm:grid-flow-col sm:gap-6 flex-col">
          <div className="flex flex-row w-full sm:flex-row flex-col gap-6 items-center w-1/2 h-auto">
            <UserImage
              onPressEditImage={onPressEditPicture}
              onPressDeleteImage={onPressDeletePicture}
              path={userDetails?.picture}
              alt="Profile Image"
            ...
            <div className="flex flex-col items-center sm:items-start justify-center">
              <Skeleton isLoaded={!detailsLoading}>
                <div className="gap-4 w-full">
                  ...
                  <div
                    className="font-bold text-xl sm:text-4xl overflow-hidden text-ellipsis whitespace-normal"
                    style={{
                      display: "-webkit-box",
                      WebkitLineClamp: 2,
        ...
        <div className="flex mt-8">
          <div>
            <Skeleton isLoaded={!detailsLoading}>
              ...
              <span className="text-lg text-muted-foreground">{aboutText}</span>
            </Skeleton>
          </div>
        </div>
      </CardBody>
